...
def fetch_detailed_market_data(coin_id="bitcoin", timeframe="1d"):
    try:
        ...
        if response.status_code == 200:
            return response.json().get("prices", None)
        return None
    except Exception:
        return None
...
def calculate_advanced_metrics(prices, timeframe="1d"):
    if not prices or len(prices) < 26:
        return get_fallback_metrics(timeframe)
    ...
    try:
        ...
        # حساب الاحتمالات الرياضية بناءً على السكور الفعلي للمحرك
        prob_up = score
        prob_down = 100 - score

        if score >= 95: decision, decision_ar, grade = "STRONG BUY", "🟢 STRONG BUY", "AAA+ 🟢 (أفضل 1%)"
        elif 85 <= score < 95: decision, decision_ar, grade = "BUY", "🟢 BUY", "AAA 🟢 (أفضل 5%)"
        elif 75 <= score < 85: decision, decision_ar, grade = "BUY", "🟢 BUY", "AA 🟢 (أفضل 10%)"
        elif 65 <= score < 75: decision, decision_ar, grade = "BUY", "🟢 BUY", "A 🟢 (جيدة)"
        elif 40 <= score < 65: decision, decision_ar, grade = "WATCH", "🟡 WATCH", "BBB 🟡 (متوسطة)"
        elif 25 <= score < 40: decision, decision_ar, grade = "WATCH", "🟡 WATCH", "BB 🟡 (ضعيفة)"
        else: decision, decision_ar, grade = "SELL", "🔴 SELL", "C 🔴 (لا تدخل)"
...
def get_fallback_metrics(timeframe):
    expiry_map = {"5m": "15 دقيقة", "1h": "3 ساعات", "4h": "12 ساعة", "1d": "24 ساعة"}
    return {
        "current_price": 0.0, "price_change_pct": 0.0, "rsi": 50, "macd_status": "Neutral ⚪",
        "ema_status": "Neutral ⚪", "score": 50, "prob_up": 50, "prob_down": 50, "decision": "WATCH", 
        "decision_ar": "Base Scan 🟡", "grade": "BBB 🟡 (بيانات احتياطية)", "market_phase": "Distribution (توزيع وتذبذب) ⚖️",
        "entry": 0.0, "stop_loss": 0.0, "tp1": 0.0, "tp2": 0.0, "tp3": 0.0, "rr": "1 : 2.0",
        "tech_consensus": "WATCH", "onchain_consensus": "WATCH", "sentiment_consensus": "WATCH",
        "reasons": "⚠️ يتم عرض البيانات الاحتياطية حالياً بسبب ضغط الطلبات على السيرفر الخارجي.",
        "valid_until": expiry_map.get(timeframe, "6 ساعات"),
        "signal_id": f"#{random.randint(80000, 99999)}",
        "generated_time": datetime.datetime.utcnow().strftime("%H:%M UTC")
    }
